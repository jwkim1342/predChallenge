...
pred.logReg = predict(logReg, newdata = valid.df, type = 'response')

pred = ifelse(pred.logReg > 0.5, 1, 0)



pred.df = data.frame(actual = valid.df$Competitive., prediction = pred, probability = p_prob)
pred.df[order(-p_prob),]
confusionMatrix(pred, valid.df$Competitive)

# C #
n.log.reg = glm(Competitive.~.-ClosePrice, data = train.df, family = 'binomial')

p2_prob = predict(n.log.reg, newdata = valid.df, type = 'response')
pred2 = ifelse(p2_prob > 0.5, 1, 0)
pred2.df = data.frame(actual = valid.df$Competitive., prediction = pred2, probability = p2_prob)
pred2.df[order(-p2_prob),]
confusionMatrix(pred2, valid.df$Competitive.)
